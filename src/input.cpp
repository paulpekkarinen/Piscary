...
void player_locationselect(level_type *level)
{
	...
	int tx=0; //note: add screen location later
	int ty=0;
	...
	while (1)
	{
		...
		if (keycode==' ' && monsterthere)
		{
			if (aptr)
			{
				display->Monster_Description((being*)aptr);
				display->Redraw(level);
				my_setcolor(C_WHITE);
				msg.newmsg("Use movement keys for location, quit with ESC or ENTER.");
			}
			else
				msg.newmsg("No info.");
		}
		else if (dir)
		{
			...
			Coord vc(rx+tx-1, ry+ty-1);
			if (gameview.Is_Visible(vc)==false)
			{
				tx=ltx;
				ty=lty;
				vc.Set(rx+tx-1, ry+ty-1); //change also vc
			}
			...
			put_char('*', CH_RED);
			gotoxy(0, MSGLINE+1);
			aptr=gameview.Get_Actor(vc);

			const int items=level->inv.Count_Items_Flat(rx+tx-1, ry+ty-1);
			monsterthere=false;
			if (aptr)
			{
				string s=monster_sprintf(aptr, true, true);
				msg.vnewmsg(C_RED, "%s. (More info with SPACE)", s.c_str());
				monsterthere=true;
			}
			else if (items>0)
			{
				if (items==1)
				{
					iptr=gameview.Get_Item(vc);
					display->Item_Info(&iptr->i, iptr->i.weight, iptr->count, "It's");
				}
				else
				{
			...
			{
				const int tt=level->Get_Terrain(vc);
				msg.vnewmsg(C_WHITE, "%s.", terrains[tt].desc);
			}

#ifdef saladir_debug
			/*
					if(level->loc[ry+ty-1][rx+tx-1].flags & CAVE_NOLIT)
					   msg.newmsg("UNLIT!!", CH_RED);
					if(level->loc[ry+ty-1][rx+tx-1].flags & CAVE_MONSTERLIGHT)
					   msg.newmsg("monsterlight!!", CH_RED);
					if (gameview.Is_Visible(vc))
					   msg.newmsg("LOS can see!!", CH_RED);
					if(level->loc[ry+ty-1][rx+tx-1].flags & CAVE_TMPLIGHT ||
					   level->loc[ry+ty-1][rx+tx-1].flags & CAVE_MONSTERLIGHT)
